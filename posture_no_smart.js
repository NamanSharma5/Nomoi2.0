...
let faceTouchWarnings = 0; // faceTouch ignored?
    ...
    if (poses.length > 1 ){
        ...
        for (i = 0 , pose_length = poses.length ; i < pose_length; i++ ){
            if (poses[i].pose.score > 0.31){
                chosenOneEyeDistance = dist(poses[i].pose.leftEye.x,poses[i].pose.leftEye.y,poses[i].pose.rightEye.x,poses[i].pose.rightEye.y)   
...
function draw() {
    ...
    scaleHeight = (window.innerHeight/900)*0.70
    ...
    if (pose){
        ...
        fill(5, 255, 255);
        if ((nose.x > rightBoundary) && (nose.x < leftBoundary) && (nose.y < bottomBoundary)){
            ellipse(nose.x,nose.y,radius);
            ellipse(eyeR.x, eyeR.y,radius);
            ellipse(eyeL.x,eyeL.y,radius);
        }

        if( (shoulderR.x > rightBoundary) && (shoulderR.y < bottomBoundary) && (shoulderR.x < leftBoundary)){
        ellipse(shoulderR.x,shoulderR.y,radius)
        }else{drawLine = 0}

        if( (shoulderL.x < leftBoundary) && (shoulderL.y < bottomBoundary) && (shoulderL.x > rightBoundary) ){
        ...
        if (fixed==1){
            ...
            if (left_hand_mode == 0){ 
                ...
                if(postureNotification == 1){
                    ...
                    if ((shoulderR.y > idealShoulderR.y)&& (shoulderR.confidence > min_shoulder_confidence)){
                ...
                if(postureNotification == 1){
                    ...
                    if ((shoulderL.y > idealShoulderL.y)&&(shoulderL.confidence > min_shoulder_confidence)){ 
        ...
        if (faceTouchActive == 1 ){ // write some code for head tilt back -- console.log(nose.y - eyeR.y)  -- have to account for screen distance 
            ...
            if ( ((leftWristDistance < 150) && (wristL.confidence > 0.4)) || ((rightWristDistance < 150) && (wristR.confidence > 0.4)) ){
                fill(255, 125, 69)
                if(wristL.y < bottomBoundary){
                    ellipse(wristL.x,wristL.y,radius)}
                if(wristR.y < bottomBoundary){
               ...
               if(faceTouchcount == 0){
                   if ((abs(faceTouchcurrentSecond - faceTouchpreviousSecond) == 1) || (faceTouchpreviousSecond == 59 && faceTouchcurrentSecond == 0)){ // do I really need abs(difference)
        ...
        if ((distance > tooCloseDistance)&&(eyeL.confidence > 0.69)&&(eyeR.confidence > 0.69)){
            if (too_close > 0){
        ...
        if (timer_on > 0){
            ...
            if (distance < 22){ 
                ...
                time_between_minute = abs(current_minute - previous_session_close_minute) 
                time_between_second = abs(current_second - previous_session_close_second) 
                ...
                session_second_difference = session_length_second-current_session_previous_second
           ...
           if (start_time_minute > 39){
            ...
            if ( ((minutes_remaining == 0) && (seconds_remaining == 0)) || (minutes_remaining > 21)){
            ...
            if (reset_timer == true){ // FUNCTION removed
                start_time_minute = current_minute
...
function calibrate (){
    ...
    if (pose){
        ...
        if (Push.Permission.get() == 'denied'){
            ...
            msg += newLine;
            msg += "Click the 'add' button next to allow and paste https://nomoi.org.uk";
            alert(msg);
        ...
        document.getElementById('perfectPosture').innerHTML= "Recalibrate ideal posture"
...
function pausePosture(){
    ...
    if(fixed == 1){
        ...
        fixed = 0;
        postureTrackerUsage = Number.parseFloat( ( parseInt(postureTrackerUsage,10) + (performance.now() - postureTrackerUsageStart ) / 60000)).toFixed(0);
    }else{
        document.getElementById('pausePosture').innerHTML = "Pause"
        fixed = 1;
        postureTrackerUsageStart = performance.now()
...
function timer(){
    ...
    if (timer_on == 0){
        ...
        screenTimerUsage = Number.parseFloat( parseInt(screenTimerUsage,10) + ((performance.now() - screenTimerUsageStart) / 60000) ).toFixed(0);        
    }
...
function tooCloseNotifications(){
    ...
    if (too_close == 0){
        ...
        document.getElementById("tooCloseButton").innerHTML = "Too Close to Screen Notifications: OFF";
...
function voiceMessage(){
    ...
    if (talking_timer == 0){
        ...
        document.getElementById("voiceMessage").innerHTML = "Talking Timer Notification: OFF";
...
function faceTouch(){
    if (faceTouchActive == 0){
        ...
        faceTouchUsage = Number.parseFloat( parseInt(faceTouchUsage,10) + ((performance.now() - faceTouchUsageStart ) / 60000) ).toFixed(0);
...
function maskPrompt(){
    if (maskPromptActive == 0){
        ...
        faceMaskUsage = Number.parseFloat( ( parseInt(faceMaskUsage,10) + (performance.now() - faceMaskUsageStart ) / 60000)).toFixed(0);
...
function notificationMode(){
    if(notificationModeOn == 1){
        ...
        document.getElementById('notificationMode').innerText ="Live Notifications: OFF"
    }else{
        notificationModeOn = 1;
        document.getElementById('notificationMode').innerText ="Live Notifications: ON"
    }
}
...
setInterval(
    ...
    faceTouchScore = faceTouchPreviousScore + Number(usageWarningScore(currentTime,faceTouchUsageStart,faceTouchWarnings));
    ...
    postureTrackerScore = postureTrackerPreviousScore + Number(usageWarningScore(currentTime,postureTrackerUsageStart,postureTrackerWarnings));
    postureTrackerScore  = scoreRange(postureTrackerScore)
    screenTimerScore = screenTimerPreviousScore + Number(usageScore(currentTime,screenTimerScore))
    screenTimerScore=scoreRange(screenTimerScore)
    screenDistanceScore = screenDistancePreviousScore + warningScore(screenDistanceWarnings);
    screenDistanceScore = scoreRange(screenDistanceScore)

    hydrationTrackerScore = hydrationTrackerPreviousScore + hydrationTrackerScoreCalculator(hydrationTrackerUsage); 
    hydrationTrackerScore = scoreRange(hydrationTrackerScore)

    ergonomicScore = Number.parseFloat(ergonomicPreviousScore+(0.25*(Number(usageWarningScore(currentTime,postureTrackerUsageStart,postureTrackerWarnings))+Number(usageScore(currentTime,screenTimerScore))+ warningScore(screenDistanceWarnings)+hydrationTrackerScoreCalculator(hydrationTrackerUsage)))).toFixed(0);
    ergonomicScore  = scoreRange(ergonomicScore)
    covid19Score = Number.parseFloat(covid19PreviousScore+(0.5*(Number(usageWarningScore(currentTime,faceTouchUsageStart,faceTouchWarnings))+warningScore(faceMaskWarnings)))).toFixed(0);
    covid19Score  = scoreRange(covid19Score)

    nomoiScore = Number.parseFloat((Number(covid19Score) + Number(ergonomicScore))*0.5).toFixed(0);
...
function drawPostureTimerGauges() {
    ...
    var options = {
      width: postureTimerGaugeWidth, height: postureTimerGaugeHeight,
      redFrom: 0, redTo: 33,
      yellowFrom:33, yellowTo: 66,
      greenFrom: 66, greenTo:110,
      minorTicks: 5
    };

    var chart = new google.visualization.Gauge(document.getElementById('postureTimerGauges'));
...
function drawHydrationDistanceGauges() {
    ...
    var options = {
      width: hydrationDistanceGaugeWidth, height: hydrationDistanceGaugesHeight,
      redFrom: 0, redTo: 33,
      yellowFrom:33, yellowTo: 66,
    ...
    var chart = new google.visualization.Gauge(document.getElementById('hydrationDistanceGauges'));
